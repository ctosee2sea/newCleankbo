...
if (!$mb['mb_id'] || !check_password($mb_password, $mb['mb_password'])) {
    // echo $mb_password.":".$mb['mb_password'];
...
if($_POST['token'] && $_POST['os']) {
    $token = trim($_POST['token']);
    $os = trim($_POST['os']);
sql_query("INSERT INTO users(Token, mb_id, os) Values ('{$token}', '{$mb['mb_id']}', '{$os}') ON DUPLICATE KEY UPDATE Token = '{$token}', mb_id = '{$mb['mb_id']}', os='{$os}'");
}
goto_url($link);
?>
