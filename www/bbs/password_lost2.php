...
$sql = " select mb_no, mb_id, mb_name, mb_nick, mb_email, mb_datetime,mb_hp from {$g5['member_table']} where mb_email = '$email' ";
$mb = sql_fetch($sql);
if (!$mb['mb_id'])
    alert('존재하지 않는 회원입니다.');
...
$href2 = G5_BBS_URL.'/password_lost_certify.php?mb_no='.$mb['mb_no'].'&mb_nonce='.$mb_nonce;
...
$sms = new SMS();
$number = str_replace("-", "", $mb['mb_hp']);
$text = "{$mb['mb_id']}님의 임시 비밀번호 변경 요청입니다. \r아래 주소를 누르시면 비밀번호가 변경됩니다.\r{$href2}\r 변경될 클린베이스볼센터 임시 비밀번호는 [{$change_password}] 입니다.";

// echo $text;
// exit();
$rr = $sms->setType("LMS")->setNumber($number)->setMsg($text)->send();


mailer($config['cf_admin_email_name'], $config['cf_admin_email'], $mb['mb_email'], $subject, $content, 1);

alert_close($email.' 메일과 휴대폰으로 회원아이디와 비밀번호를 인증할 수 있는 메일이 발송 되었습니다.\\n\\n메일을 확인하여 주십시오.');
?>
