...
<?php
	$playtime = "select * from cbc_lecture";
	$playresult = sql_query($playtime);
	$playtime =[];
	for($i=0;$playrow = sql_fetch_array($playresult);$i++){
	$time    = $playrow[lt_playtime];
	$seconds = 0;
	$parts   = explode(':', $time);
	$seconds += $parts[0] * 60;
	$seconds += $parts[1];
	$playtime[$playrow[lt_subject]] = $seconds;
	// print_r($playrow[lt_subject]."+".$playrow[lt_playtime]."+".$seconds);
}

	foreach($result as $tmp){
		// print_r($tmp);
			$realtime=strtotime($tmp[etime]) - strtotime($tmp[stime]);
			?>
			<div class="item">
				<div class="floatL left"><?=$tmp['lt_subject']?></div>
				<div class="floatR right">
					<?php 
// echo($playtime[$tmp[lt_subject]]."-".$realtime);
						// if(abs(ceil($realtime) - ceil($tmp[ctime])) < 30 || $realtime > $tmp[ctime]){
						if(abs(ceil($realtime) - ceil($playtime[$tmp[lt_subject]])) < 30 || $playtime[$tmp[lt_subject]] < $realtime){
					?>
					<!-- <div class="date floatL p_date"><?=substr($tmp['etime'],0,10);?> (sec:<?=$realtime?>)</div> -->
					<div class= "btn_on floatL p_btn">이수</div>
					<?php }else{?>
