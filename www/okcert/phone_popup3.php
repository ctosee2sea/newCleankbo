...
<?php
    ...
    //' 라이센스 파일
    //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	$license = "/home/cleankbo/".$CP_CD."_IDS_01_".$target."_AES_license.dat";
...
<script language="javascript" type="text/javascript" >
	function fncOpenerSubmit() {
		var $form = $("form#formResult");
		// $form.find("input[name="++"]").val()
		$form.find("input[name=CP_CD]").val("<?=$CP_CD?>");
		$form.find("input[name=TX_SEQ_NO]").val("<?=$TX_SEQ_NO?>");
		$form.find("input[name=RSLT_CD]").val("<?=$RSLT_CD?>");
		$form.find("input[name=RSLT_MSG]").val("<?=$RSLT_MSG?>");
		$form.find("input[name=RETURN_MSG]").val("<?=$RETURN_MSG?>");
...
?>
		...
		$form.find("input[name=RSLT_NAME]").val("<?=$RSLT_NAME?>");
		$form.find("input[name=RSLT_BIRTHDAY]").val("<?=$RSLT_BIRTHDAY?>");
		$form.find("input[name=RSLT_SEX_CD]").val("<?=$RSLT_SEX_CD?>");
		$form.find("input[name=RSLT_NTV_FRNR_CD]").val("<?=$RSLT_NTV_FRNR_CD?>");
		
		$form.find("input[name=DI]").val("<?=$DI?>");
		$form.find("input[name=CI]").val("<?=$CI?>");
		$form.find("input[name=CI_UPDATE]").val("<?=$CI_UPDATE?>");
		$form.find("input[name=TEL_COM_CD]").val("<?=$TEL_COM_CD?>");
		$form.find("input[name=TEL_NO]").val("<?=$TEL_NO?>");
...
?>	
		...
		$form.attr("action","phone_popup4.php");
		$form.submit();
	...
	<form name="kcbResultForm" method="post" id="formResult">
